...
<v-container class="pa-0 my-0">
        <v-layout  class="my-0 mt-2" >
            ...
            <div style="border-radius: 5px; border:1px solid blue;min-height:180px" class="ma-1 pa-5" v-for="person in schedule2019.slice().reverse()" :key="person.sessionId">
                     <v-layout row wrap justify-center align-self-center>
                         ...
                         <v-flex xs12 sm2 md2>
                            <v-chip color="green" class="ma-1 left " outline small>talk time: {{person.sessionStartTime}} </v-chip>
...
export default {
  ...
  created () {
    db.collection("schedule2019").where("track", "==",'cloud')
    //  .orderBy("", "desc")
    .onSnapshot(res => {
