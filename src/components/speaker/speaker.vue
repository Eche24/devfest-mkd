...
<v-container>
    ...
    <v-layout row wrap  fill-height >
     <v-flex xs12 sm6 md4 lg3 v-for="person in speakers" :key="person.id">
      <v-card class="text-xs-center ma-3 "  max-width="344">
       <v-responsive class="pt-4">
          <v-avatar size="129" class="grey lighten-2">
                        <v-img
       ...
       <v-card-text>
         ...
         <div class="grey--text">{{person.company}}</div> 
         <div class="grey--text">{{person.desc}}</div> 
         <p class="mt-2" > 
           talk time :
           {{person.time}}
           </p>
...
export default {
  ...
  created () {
    db.collection('speakers').orderBy("name", "asc").onSnapshot(res => {
